...
class UAVEnv(gym.Env):
    ...
    def __init__(self, render_mode=True):
        ...
        # observation: dx, dy, dz, dist, yaw, d_yaw
        high = np.array([
            1,
            1,
            1,
        ...
        self.action_space = spaces.Box(
            low=np.array([-vmax, -vmax, -vmax, -yaw_rate_max], dtype=np.float32),
            high=np.array([vmax, vmax, vmax, yaw_rate_max], dtype=np.float32),
            dtype=np.float32
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        while True:
            ...
            g_yaw = float(np.random.uniform(-np.pi, np.pi))
        ...
        yaw_e = np.arctan2(np.sin(yaw_e), np.cos(yaw_e))   # wrap to [-π,π]
    ...
    def step(self, action):
        ...
        alpha = 0.5
        self.vel = alpha*v_des[:3] + (1-alpha)*self.vel
        ...
        reward += -0.45*abs(self.pos[2]-self.goal[2])
        reward += -0.01*np.linalg.norm(v_des)

        yaw_e = self.yaw - self.goal_yaw
        yaw_e = np.arctan2(np.sin(yaw_e), np.cos(yaw_e))
        if dist_to_goal < 1.0:
            reward += 5*(1- ((yaw_e)**2 )/ (np.pi**2))
        else:
            reward += -1*abs(action[3])
        ...
        if collision:
            ...
            info["collision"] = True
        elif dist_to_goal < 0.05 and abs(yaw_e) < 0.1:
            reward += 500.0
            done = True
            info["success"] = True
    ...
    def render(self):
        ...
        ax_xy.arrow(
            x, y,
            yaw_len * np.cos(self.yaw),
            yaw_len * np.sin(self.yaw),
            head_width=0.2, head_length=0.2,
            fc='blue', ec='blue'
        )
        ax_xy.arrow(
            self.goal[0], self.goal[1],
            yaw_len * np.cos(self.goal_yaw),
            yaw_len * np.sin(self.goal_yaw),
            head_width=0.2, head_length=0.2,
            fc='green', ec='green'
        )
